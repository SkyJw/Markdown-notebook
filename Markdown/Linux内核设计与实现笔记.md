...
- 用户进程被抢占，重新调度，新调度的进程与被抢占的进程有可能进入同一个临界区

- 对称多处理器情况，多个处理器之间可以真并发运行，从而产生竞争
...
​	如果有其他执行线程可以访问到这些数据，那么就给这些数据加上某种形式的锁。记住：要给数据而不是给代码加锁

### 9.3 死锁

​	死锁的调节：要有一个或多个执行线程和一个或多个资源，每个线程都在等待其中的一个资源，但是所有的资源都已经被占用了。这会导致所有的线程都在相互等待，但它们永远不会释放已经占有的资源，于是任何线程都无法继续

​	最简单的死锁例子就是自死锁，即一个执行线程试图去获得一个自己已经持有的锁
...
​    释放锁的顺序是和死锁无关的，但是最好还是以获得锁的相反顺序来释放锁

### 9.4 争用和扩展性

​	锁的争用简称争用。是指当锁正在被占用时，有其他线程试图获得该锁。当有多个其他线程在等待获得某个锁时，无疑会降低系统性能

​	加锁方式的可扩展性，和锁的粒度相关，处理器的数量越多，锁区域的粒度应该越细，这样就可以发挥出多处理器的优势，减少因为锁而等待的时间。但是这种情况转移到处理器数量少的机器上运行时，这种小粒度的锁则会造成很多不必要的开销，从而降低系统性能。因为核少，能够同时处理的能力也就小，没有必要使用粒度太小的锁。<font color=red>当锁争用严重时，加锁太粗会降低可扩展性；而锁争用不明显时，加锁过细会加大系统的开销，带来浪费</font>
...
​	当应用场景为读者-写者，或是简单的生产者-消费者时，可以使用读-写自旋锁进行操作。读-写自旋锁中，单纯的读操作可以并发，写与写、写与读直接均为互斥。这种锁往往偏向读者，当读者进程大量运行时，有可能导致写者进程饥饿。

### 10.4 信号量

​	Linux中的信号量是一种睡眠锁。当一个任务去获取一个已经被占用的信号量时，信号量会将其推进到一个等待队列，让其睡眠。

​	信号量睡眠特性的一些有意思结论：

- 争用信号量会睡眠，所以信号量适用于会被长时间持有的应用场景

- 反过来说，短时间被锁就不适用信号量，因为信号量开销大，有可能睡眠、维护队列、唤醒等操作的时间比锁被占用的时间还要长

- 因为锁被争用时会睡眠，所以信号量只能在进程上下文使用，因为在中断上下文中是不能进行调度的

- 持有信号量时是可以睡眠的，因为当其他进程尝试获取你持有的信号量时，它也会睡眠，从而并不会造成死锁

- 在你占用信号量的同时不能占用自旋锁。因为在你等待信号量时可能会睡眠，从而造成自旋锁死锁

​	在使用信号量的大多数情况中，并没有多大的余地去选择其他同步方法。在内核空间和用户空间同步时，你的代码会需要睡眠（<font color=blue>如使用` copy_to_usr()`和` copy_from_usr() `方法</font>），此时使用信号量是唯一的选择

#### 10.4.1 计数信号量和二值信号量

​	Linux内核中基本上用到的都是互斥信号量

#### 10.4.2 创建和初始化信号量

​	静态声明信号量，其中name是信号量的变量名，count是信号量的使用数量：
...
​	更常见的情况是，信号量作为一个大数据结构的一个成员，进行动态创建。此时，只有指向该动态创建的信号量的间接指针，可以试用如下函数来对它进行初始化：
...
​	<font color=red>思考：这里的初始化方式为什么是指针，不太明白。在自己的使用中，往往信号量作为结构体的一个成员，此时是直接定义一个信号量` struct semaphore name `而非是定义一个指针</font>

#### 10.4.3 使用信号量

​	函数down_interruptible()试图获取指定信号量，如果无信号量可用，则进入可被信号唤醒的睡眠（<font color=red>此处的信号，和信号量的信号不是同一个概念</font>）。函数down()，若无信号量可用，则会进入不可被信号唤醒的睡眠。down_interruptible()若被信号唤醒，则返回-EINTR，在使用处应加以处理。down_interruptible()使用更加普遍和正确。
